...
flags.DEFINE_boolean("image_obs", False, "Whether to use image observations")
...
def actor(agent: SACAgent | DrQAgent, data_store, env, sampling_rng):
    """
...
def learner(
        rng, 
        agent: SACAgent | DrQAgent, 
        replay_buffer: ReplayBufferDataStore | MemoryEfficientReplayBufferDataStore, 
        demo_buffer: Union[ReplayBufferDataStore, MemoryEfficientReplayBufferDataStore, None] = None
    ):
    """
    ...
    if not FLAGS.image_obs:
        sample_args = {"batch_size": single_buffer_batch_size}
    else:
        sample_args = {
...
def player(agent: SACAgent | DrQAgent, env, sampling_rng):
    """
    This is the actor loop, which runs when "--actor" is set to True.
    """
    success_counter = 0
...
def main(_):
    ...
    if FLAGS.render:
        env = gym.make(FLAGS.env, render_mode="human", image_obs=FLAGS.image_obs)
    else:
        env = gym.make(FLAGS.env, image_obs=FLAGS.image_obs)
